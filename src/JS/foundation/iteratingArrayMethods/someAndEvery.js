const description = 'Эти методы используются для проверки массива на полное совпадение или частичное';
...
const methodSomeAndEvery = function (data, type) {
    switch (type) {
        case 'some':
            return data.some(user => user.gender === 'male');
        case 'every':
            return data.every(user => user.gender === 'male');
        default:
            return data
    }
}
...
console.log(methodSomeAndEvery(users, 'every'));

// some - возвращает TRUE если хоть один элемент удовлетворяет условие
// every - возвращает TRUE если все элементы удовлетворяют условие
