const description = 'Возвращает массив из переданных в колбек элементов';
...
const methodMap = function (data, type) {
    switch (type) {
        case 'cleanArray':
            return data.map(item => item.name);
        case 'randomKeyObject':
            return data.map(item => ({name: item.name, age: item.age, status: item.isActive}));
        case 'creatingTemplate':
            return data.map(item => {
                return `My name is ${item.name}, my ${item.age} years. I have ${item.isActive}, status.`
            });
        default:
            return data
    }
...
console.log(methodMap(users, 'creatingTemplate'));

// cleanArray - обычный массив
// randomKeyObject - массив из объектов с произвольными ключами
// creatingTemplate - создание шаблона на основе данных из массива
