const mainFunction = function () {
    ...
    // filter
    value = users.filter((user) => user.age < 30);
    value = users.filter((user) => user.isActive);

    // map
    value = users.map(user => user.registered);
    ...
    // reduce
    value = users.reduce((acc, user) => acc + user.balance, 0);
    // Формирование объекта объектов
    ...
    // some/every
    // some - если хоть один true
    value = users.some(user => user.gender === 'male');
    // every
    // every - если все true
    value = users.every(user => user.age > 18);

    // find
    value = users.find(user => user.name === 'Lucile Finley');

    // sort
    const strArr = ['Denis', 'Bill', 'Alina'];
    value = strArr.sort();

    const numArr = [10, 7, 44, 32];
    value = numArr.sort((prev, next) => prev - next);
